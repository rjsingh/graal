...
public class ReplacementsImpl implements Replacements {
    ...
    public StructuredGraph getMethodSubstitution(ResolvedJavaMethod original) {
        ...
        if (graph == null) {
            inliningPolicy(substitute);
            StructuredGraph sg = makeGraph(substitute, original, inliningPolicy(substitute));
            graphs.putIfAbsent(substitute, sg);
    ...
    protected class GraphMaker {
        ...
        boolean parallelMethod;

        protected GraphMaker(ResolvedJavaMethod substitute, ResolvedJavaMethod original) {
            this.method = substitute;
            this.original = original;
            // HACK BY RANJTEET SINGH - REMEMBER WE CAN'T USE TYPES FROM JAVA.UTIL.* IN
            // REPLACEMENT METHOD. THIS FIXES THE STACKOVERFLOW.
            if (original != null)
                parallelMethod = original.getAnnotation(Parallel.class) != null;
        }
